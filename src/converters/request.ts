...
export function convertRequestToOpenAI(
  ...
  if (toolFormat === 'native' && anthropicRequest.tools && anthropicRequest.tools.length > 0) {
    logger.debug('Request converted with native tools', {
      toolCount: anthropicRequest.tools.length,
      hasToolChoice: !!anthropicRequest.tool_choice,
    });
  ...
  logger.debug('Request transformation complete', {
    model: targetModel,
  ...
  for (const block of blocks) {
    if (block.type === 'text') {
      ...
      if (ctx.seenIds.has(toolUse.id)) {
        ...
        logger.debug('Duplicate tool ID remapped', { originalId: toolUse.id, newId: idToUse });
      }
